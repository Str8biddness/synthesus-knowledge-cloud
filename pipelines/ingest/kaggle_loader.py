...
def _iter_jeopardy_entries(files: List[Path]) -> Generator[KnowledgeEntry, None, None]:
    """Iterates through Jeopardy TSV files and yields KnowledgeEntry objects.

    Args:
        files: A list of Paths to Jeopardy TSV files.

    Yields:
        KnowledgeEntry objects containing cleaned question, answer, and metadata.
    """
    row_idx = 0
    for f in files:
...
def _iter_conceptnet_entries(
    ...
    """Iterates through ConceptNet gzipped CSV assertions and yields KnowledgeEntry objects.

    Filters for English (/c/en/) nodes and the specified relation types.

    Args:
        gz_path: Path to the gzipped ConceptNet assertions file.
        relations: A list of relation labels (e.g., ['IsA', 'PartOf']) to include.

    Yields:
        KnowledgeEntry objects containing mapped natural-language facts.
    """
