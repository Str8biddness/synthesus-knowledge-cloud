...
Usage:
    python pipelines/build/run_population.py
    python pipelines/build/run_population.py --max 10000
    python pipelines/build/run_population.py --sample-jeopardy 1000 --sample-conceptnet 1000
"""
...
from pipelines.build.kn_populator import KNPopulator
...
def main() -> None:
    """Main CLI entry point for the knowledge population tool.

    Parses command-line arguments, initializes the embedder and populator,
    loads datasets (Jeopardy, ConceptNet), populates the FAISS index and
    KNDB, and optionally runs a semantic search test to verify the results.
    """
